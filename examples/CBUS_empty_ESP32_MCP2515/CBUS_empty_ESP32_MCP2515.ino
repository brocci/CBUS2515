...
const byte LED_GRN = D4;            // CBUS green SLiM LED pin
const byte LED_YLW = D5;            // CBUS yellow FLiM LED pin
const byte SWITCH0 = D6;            // CBUS push button switch pin

// ESP32 SPI peripheral pins - change as desired
#define VSPI_MISO 2
#define VSPI_MOSI 4
#define VSPI_SCLK 0
#define VSPI_SS   33
...
void setupCBUS() {
  ...
  //modconfig.setEEPROMtype(EEPROM_USES_FLASH);
  modconfig.begin();
  ...
  // register our CAN frame handler, to process *every* CAN frame received
  CBUS.setFrameHandler(framehandler);

  // register our CAN frame handler, to process *every* CAN frame transmitted
  CBUS.setTransmitHandler(transmithandler);

  // set CBUS LEDs to indicate the current mode
  CBUS.indicateMode(modconfig.FLiM);

  // configure the selected SPI peripheral and non-default pins
  spi = new SPIClass(VSPI);
  spi->begin(VSPI_SCLK, VSPI_MISO, VSPI_MOSI, VSPI_SS);  // SCLK, MISO, MOSI, SS

  // configure and start CAN bus and CBUS message processing
  CBUS.setNumBuffers(32, 8);     // more buffers = more memory used, fewer = less
  CBUS.setOscFreq(16000000UL);   // select the crystal frequency of the CAN module
  CBUS.setPins(D7, 2);           // select pins for CAN bus CE and interrupt connections

  if (!CBUS.begin(false, *spi)) {
...
//
/// user-defined transmit handler function
/// called from the CBUS library for *every* CAN frame transmitted
/// it receives a pointer to the received CAN frame
//
